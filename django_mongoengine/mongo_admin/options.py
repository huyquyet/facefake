...
#                                              ImageField)
from mongoengine import (DateTimeField, URLField, IntField,
                         ListField, EmbeddedDocumentField,
                         ReferenceField, StringField, FileField,
                         ImageField)
...
class BaseDocumentAdmin(object):
    ...
    def formfield_for_manytomany(self, db_field, request=None, **kwargs):
        ...
        if db_field.name in self.raw_id_fields:
            ...
            kwargs['widget'] = widgets.FilteredSelectMultiple(pretty_name(db_field.name),
...
class DocumentAdmin(BaseDocumentAdmin):
    ...
    def get_inline_instances(self):
        for f in self.document._fields.itervalues():
            if not (isinstance(f, ListField) and isinstance(getattr(f, 'field', None),
                                                            EmbeddedDocumentField)) and not isinstance(f,
    ...
    def response_add(self, request, obj, post_url_continue='../%s/'):
        ...
        msg = _('The %(name)s "%(obj)s" was added successfully.') % {'name': force_text(opts.verbose_name),
                                                                     'obj': force_text(obj)}
    ...
    def __init__(self, field, parent_document, admin_site):
        ...
        if self.verbose_name is None:
            self.verbose_name = "Field: %s (Document: %s)" % (
                capfirst(field.name), self.document._admin_opts.verbose_name)

        if self.verbose_name_plural is None:
            self.verbose_name_plural = "Field: %s (Document:  %s)" % (
                capfirst(field.name), self.document._admin_opts.verbose_name_plural)
